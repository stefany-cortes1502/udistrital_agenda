...
// @Success 201 {int} models.Parametros
// @Failure 403 body is empty
// @router / [post]
func (c *ParametrosController) Post() {
	var v models.Parametros
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddParametros(&v); err == nil {
...
// @router /:id [get]
func (c *ParametrosController) GetOne() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v, err := models.GetParametrosById(id)
	if err != nil {
		logs.Error(err)
...
func (c *ParametrosController) GetAll() {
	...
	l, err := models.GetAllParametros(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
...
// @router /:id [put]
func (c *ParametrosController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.Parametros{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateParametrosById(&v); err == nil {
			c.Data["json"] = map[string]interface{}{"Success": true, "Status": "201", "Message": "Registro completado", "Data": v}
		} else {
			logs.Error(err)
...
// @Failure 403 id is empty
// @router /:id [delete]
func (c *ParametrosController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteParametros(id); err == nil {
