...
type TelefonosController struct {
	beego.Controller
}

// URLMapping ...
func (c *TelefonosController) URLMapping() {
...
// @Param	body		body 	models.Telefonos	true		"body for Telefonos content"
// @Success 201 {int} models.Telefonos
// @Failure 403 body is empty
// @router / [post]
func (c *TelefonosController) Post() {
	var v models.Telefonos
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddTelefonos(&v); err == nil {
...
// @Success 200 {object} models.Telefonos
// @Failure 403 :id is empty
// @router /:id [get]
func (c *TelefonosController) GetOne() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v, err := models.GetTelefonosById(id)
	if err != nil {
		logs.Error(err)
...
// @Failure 403
// @router / [get]
func (c *TelefonosController) GetAll() {
	var fields []string
	var sortby []string
	var order []string
	...
	l, err := models.GetAllTelefonos(query, fields, sortby, order, offset, limit)
	if err != nil {
		logs.Error(err)
...
// @router /:id [put]
func (c *TelefonosController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.Telefonos{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateTelefonosById(&v); err == nil {
			c.Data["json"] = map[string]interface{}{"Success": true, "Status": "201", "Message": "Registro completado", "Data": v}
		} else {
			logs.Error(err)
...
// @Failure 403 id is empty
// @router /:id [delete]
func (c *TelefonosController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteTelefonos(id); err == nil {
